...
impl GsmMessageData {
    ...
    pub fn decode_message(&self) -> PDUResult<DecodedMessage> {
        ...
        if self.udh {
            if self.bytes.len() < 1 {
                Err(PDUError::InvalidPdu("UDHI specified, but no data"))?
            }
            let udhl = self.bytes[0] as usize;
            padding = 7 - (((udhl + 1) * 8) % 7);
            start = udhl + 1;
            if self.bytes.len() < start {
                Err(PDUError::InvalidPdu("UDHL goes past end of data"))?
            }
        ...
        match self.encoding {
            ...
            x => Err(PDUError::UnsupportedEncoding(x, self.bytes.clone()))
        }
    }
    ...
    pub fn encode_message(msg: &str) -> Vec<GsmMessageData> {
        ...
        if let Some(buf) = try_gsm_encode_string(msg) {
